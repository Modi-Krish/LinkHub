...
export const getPublicProfile = async (username: string, isPreview: boolean = false) => {
  let profile;
  if (isPreview) {
    profile = await BioProfile.findOne({ username });
  } else {
    profile = await BioProfile.findOneAndUpdate(
      { username },
      { $inc: { views: 1 } },
      { new: true }
    );
